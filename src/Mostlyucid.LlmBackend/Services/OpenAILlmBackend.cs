...
public class OpenAILlmBackend : BaseLlmBackend
{
    ...
    {
        // Convert to chat request for modern API
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return CreateErrorResponse($"OpenAI API returned {response.StatusCode}: {errorContent}");
            }
            ...
            var modelUsed = root.GetProperty("model").GetString();
