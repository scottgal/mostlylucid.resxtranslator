...
public class OllamaLlmBackend : BaseLlmBackend
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var requestBody = new
            {
                ...
                options = new
                {
                    num_predict = request.MaxTokens ?? Config.MaxOutputTokens ?? 2000,
                    top_p = request.TopP,
            ...
            var modelUsed = root.TryGetProperty("model", out var model) ? model.GetString() : null;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            stopwatch.Stop();
            return CreateErrorResponse($"Ollama chat request failed: {ex.Message}", ex);
        }
    }
}
